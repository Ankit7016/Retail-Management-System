...
public class RetailStore {
    ...
    public static void adminDisplayProducts() {
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            Product tmp = products.get(i);
            System.out.println(tmp.getProductID() + "\t" + tmp.getProductName() + "\t" + tmp.getQty() + "\t"
                    + tmp.getReturnPeriod() + "\t" + tmp.getBasePrice() + "\t" + tmp.getSellPrice());
    ...
    public static void displayProducts() {
        ...
        for (int i = 0; i < products.size(); i++) {
            ...
            System.out.println(pro.getProductID() + "\t" + pro.getProductName() + "\t" + pro.getQty() + "\t"
                    + pro.getReturnPeriod() + "\t" + sellPrice);
    ...
    public static void purchaseDetail() {
        ...
        System.out.println(
                "OrderID\tProduct ID \tName  \tQty   \tReturnPeriod  \tSell price  \tIssuedate   \tReturn Date");
        ...
        for (int i = 0; i < PurchaseDB.size(); i++) {
            ...
            if (UserOperations.PUser.getUsername() == pd.getUsername())
                System.out.println(pd.cnt + "\t" + pd.getProductID() + "\t" + pd.getProductName() + "\t" + pd.getQty()
                        + "\t" + pd.getReturnPeriod() + "\t" + pd.getSellPrice() + "\t" + pd.getIssueDate() + "\t"
                        + (pd.isReturned ? pd.returnDate : "NULL"));
    ...
    public static void cancelPurchase() {
        System.out.println("------------------ Purchase Detail -----------------------");
        System.out.println(
                "order ID\tProduct ID \tName  \tQty   \tReturnPeriod  \tSell price  \tIssuedate   \tReturn Date");

        for (int i = 0; i < PurchaseDB.size(); i++) {

            PurchaseDetail pd = PurchaseDB.get(i);
            if (UserOperations.PUser.getUsername() == pd.getUsername() && !pd.isReturned)
                System.out.println(pd.getProductID() + "\t" + pd.getProductName() + "\t" + pd.getQty() + "\t"
                        + pd.getReturnPeriod() + "\t" + pd.getSellPrice() + "\t" + pd.getIssueDate() + "\t"
                        + pd.returnDate);
